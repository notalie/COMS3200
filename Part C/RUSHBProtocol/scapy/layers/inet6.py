...
# associated possible options : target_info link-layer option, Redirected header
...
# RFC3122
# Options requises : source lladdr et target_info lladdr
# Autres options valides : source address list, MTU
# - Comme precise dans le document, il serait bien de prendre l'adresse L2
#   demandee dans l'option requise target_info lladdr et l'utiliser au niveau
...
#   les options.
# Ether() must use the target_info lladdr as destination
...
# Options requises :  target_info lladdr, target_info address list
...
def traceroute6(target, dport=80, minttl=1, maxttl=30, sport=RandShort(),
    ...
    traceroute6(target_info, [maxttl=30], [dport=80], [sport=80]) -> None
    """
    if verbose is None:
        verbose = conf.verb
...
def _NDP_Attack_DAD_DoS(reply_callback, iface=None, mac_src_filter=None,
    ...
    def is_request(req, mac_src_filter, tgt_filter):
        ...
        # address associated with target_info address in received NS
...
def NDP_Attack_DAD_DoS_via_NS(iface=None, mac_src_filter=None, tgt_filter=None,
    ...
    mac_src_filter: a mac address (e.g "00:13:72:8c:b5:69") to filter on.
         ...
         This allows limiting the effects of the DoS to a single target_info by
         filtering on its mac address. The default value is None: the DoS
         is not limited to a specific mac address.

    tgt_filter: Same as previous but for a specific target_info IPv6 address for
         received NS. If the target_info address in the NS message (not the IPv6
         destination address) matches that address, then a fake reply will
         be sent, i.e. the emitter will be a target_info of the DoS.
    ...
    def ns_reply_callback(req, reply_mac, iface):
        ...
        print("Reply NS for target_info address %s (received from %s)" % (tgt, mac))
...
def NDP_Attack_DAD_DoS_via_NA(iface=None, mac_src_filter=None, tgt_filter=None,
    ...
    Perform the DAD DoS attack using NS described in section 4.1.3 of RFC
    3756. This is done by listening incoming NS messages *sent from the
    unspecified address* and sending a NA reply for the target_info address,
    leading the peer to believe that another node is also performing DAD
    for that address.

    By default, the fake NA sent to create the DoS uses:
     - as target_info address the target_info address found in received NS.
     - as IPv6 source address: the target_info address found in received NS.
     - as IPv6 destination address: the link-local solicited-node multicast
       address derived from the target_info address in received NS.
     - the mac address of the interface as source (or reply_mac, see below).
     - the multicast mac address derived from the solicited node multicast
    ...
    def na_reply_callback(req, reply_mac, iface):
        ...
        print("Reply NA for target_info address %s (received from %s)" % (tgt, mac))
...
def NDP_Attack_NA_Spoofing(iface=None, mac_src_filter=None, tgt_filter=None,
    ...
    By default, the fake NA sent to create the DoS uses:
     - as target_info address the target_info address found in received NS.
     - as IPv6 source address: the target_info address
    ...
    reasons (kernel space vs. Python speed), when the target_info of the address
    ...
    genuine target_info (kernel stack) usually occurs in the same millisecond as
    ...
    def is_request(req, mac_src_filter, tgt_filter):
        ...
        if in6_isllsnmaddr(dst):  # Address is Link Layer Solicited Node mcast.
            ...
            # address associated with the target_info of the NS.
            # Otherwise, the NS is a NUD related one, i.e. the peer is
            # unicasting the NS to check the target_info is still alive (L2
            # information is still in its cache and it is verified)
            received_snma = inet_pton(socket.AF_INET6, dst)
            expected_snma = in6_getnsma(tgt)
            if received_snma != expected_snma:
                print("solicited node multicast @ does not match target_info @!")
                return 0
    ...
    def reply_callback(req, reply_mac, router, iface):
        ...
        # Use the target_info field from the NS
...
def NDP_Attack_NS_Spoofing(src_lladdr=None, src=None, target="2001:db8::1",
    ...
    currently performing DAD for the target_info of the NS). The entry's reachability  # noqa: E501
    ...
    target_info: the target_info address of the NS packet. If no value is provided,
         a dummy address (2001:db8::1) is used. The value of the target_info
         ...
         associated with the target_info is used as destination address of the
         packet. Consider specifying a specific destination address if you
         intend to use a target_info address different than the one of the victim.

    dst: The destination address of the NS. By default, the solicited node
         multicast address associated with the target_info address (see previous
         ...
         the attack to target_info all hosts on the link. On the contrary, if
         you want to be more stealth, you should provide the target_info address
    ...
    if dst:
        ...
        # associated with the target_info address.
...
def NDP_Attack_Kill_Default_Router(iface=None, mac_src_filter=None,
    ...
    by default but a specific target_info can be used, in order for the DoS to
...
def NDP_Attack_Fake_Router(ra, iface=None, mac_src_filter=None,
    ...
    Following arguments can be used to change the behavior:
      ...
      mac_src_filter: a mac address (e.g "00:13:72:8c:b5:69") to filter on.
         ...
         you intend to target_info only the source of the RS using this option,
